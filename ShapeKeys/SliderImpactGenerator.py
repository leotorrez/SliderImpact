...
def main():
    ...
    parser.add_argument("--directory", default = os.getcwd(), help="Directory to search for the files")
    parser.add_argument("-r", "--result", default = '', help="Directory to save the generated files")
    parser.add_argument("-n", "--names", default = [], action="store", nargs='*', help="Names of the sliders")
    ...
    ns = args.noside
    if args.result == '':
        args.result = os.path.join(args.directory, 'Result')
    resourcedir = os.path.join(args.result, 'resources')
    inis = [os.path.join(root, filename) for root, dirs, files in os.walk(args.directory) for filename in files 
            if filename.lower().endswith('.ini')]
    if len(inis) == 0:
        raise ValueError("ERROR: Could not find any .ini files in the directory")
    mainini = inis[0]
    charaname = os.path.basename(mainini)[:-4]
    posbuffers = [os.path.join(root, filename) for root, dirs, files in os.walk(args.directory) for filename in files 
                if filename.lower().startswith(f'{charaname}Position'.lower())]
    baseposition = posbuffers.pop(0) if len(posbuffers) > 0 else None
            
    if len(posbuffers) == 0 or baseposition is None:
        raise ValueError("ERROR: Could not find the necessary files in the directory.")

    numberoffiles = len(posbuffers)

    if args.order and numberoffiles >= 2:
        for i, posbuffer in enumerate(posbuffers):
            print(f"\t{i}:  {os.path.basename(posbuffer)}")
    ...
    if args.names == []:
        args.names = [f"Slider{i+1}" for i in range(len(posbuffers))]
    
    if len(args.names) < len(posbuffers):
        raise ValueError("ERROR: Please enter the same number of names as the number of mods")
    elif len(args.names) > len(posbuffers):
        print("WARNING: More names than the number of mods, the extra names will be ignored")
        args.names = args.names[:len(posbuffers)]

    print(numberoffiles, charaname, os.path.basename(baseposition), [os.path.basename(pos) for pos in posbuffers], args.names)
    outputini = os.path.join(args.result, f'{charaname}.ini')
    if not os.path.exists(args.result):
        try:
            os.makedirs(args.result)
        except:
            print('Could not generate Result Folder.')
    if not os.path.exists(resourcedir):
        try:
            os.makedirs(resourcedir)
        except:
            print('Could not generate Resources Folder.')
    CopyBasefiles(args.result, os.path.dirname(inis[0]))
    WriteINIFile(mainini, outputini, charaname, args)
    MoveAndRenamePositions(args.result, baseposition, posbuffers, args.names, charaname)    
    GenerateShaderFiles(resourcedir)
    GenerateTextureFiles(resourcedir)
    for i in range(numberoffiles):
        if not ns:
            GenerateIconFiles(i+1, resourcedir)
        if text:
            GenerateTextFiles(i+1, resourcedir)
    if title:
        GenerateTextFiles('Title', resourcedir)
    if author:
        GenerateTextFiles('Author', resourcedir)
    print("Done! C₁₂H₂₂O₁₁")
...
def CopyBasefiles(output, origindir):
    for root, dirs, files in os.walk(origindir):
        for filename in files:
            if 'position.buf' not in filename.lower():
                shutil.copy(os.path.join(root, filename), output)

def MoveAndRenamePositions(output, baseposition, posbuffers, names, charaname):
    for i, posbuffer in enumerate(posbuffers):
        shutil.copy(posbuffer, os.path.join(output, f'{charaname}Position{names[i]}.buf'))
    shutil.copy(baseposition, os.path.join(output, f'{charaname}PositionBase.buf'))

def GenerateIconFiles(n, output):
    ...
    try:
        with open(os.path.join(output, 'Icon{n}.png'),'xb') as png:
...
def GenerateTextFiles(n, output):
    ...
    try:
        with open(os.path.join(output, 'textResource{n}.png'),'xb') as png:
...
def GenerateTextureFiles(output):
...
def GenerateShaderFiles(output):
    try: 
        with open(os.path.join(output, 'ShapeKey.hlsl'), 'x') as f:
            f.write(
'''// **** SHAPE KEY SHADER ****
...
''')
    ...
    try:
        with open(os.path.join(output, 'UIElement.hlsl'), 'x') as f:
            f.write(
...
def WriteINIFile(inputini, outputini, charaname, args):
    numberoffiles = len(args.names)
...
cs-t51 = copy Resource{charaname}Position{args.names[i]}
x88 = $Key{i+1}*$scale
...
type = Buffer
stride = 40
filename = {charaname}PositionBase.buf
{''.join([
f'''[Resource{charaname}Position{args.names[i]}]
type = Buffer
stride = 40
filename = {charaname}Position{args.names[i]}.buf
''' for i in range(numberoffiles)
])}
[KeyToggleMenu]
